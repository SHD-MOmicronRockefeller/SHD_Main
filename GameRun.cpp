...
int GameRun::isEnd(int _isEnd_Num)
{
    ...
    for (int i = 1; i <= this->m_MapLen; i++)
    {
        ...
        {
            if (this->blockArray.at(i).at(j).m_IsOpen == false)
            {
                addtion++;
            }
        }
...
int GameRun::shoeMap(int condition)
{
    // 0 = 游戏进行
    // 1 = 游戏结束
    // 2 = 游戏胜利
    ...
    for (int i = 1; i <= this->m_MapLen; i++)
    {
        ...
        for (int j = 1; j <= this->m_MapWid; j++)
        {
            if (this->blockArray.at(i).at(j).m_IsThunder == false)
            {
                ...
                else
                {
                    if (this->blockArray.at(i).at(j).m_IsOpen == false)
                        cout << setw(2) << "M";
                    if (this->blockArray.at(i).at(j).m_IsOpen == true)
                    {
                        if (this->blockArray.at(i).at(j).m_AboutTunderNum == 0)
                            cout << setw(2) << "-";
                        else
            ...
            if (this->blockArray.at(i).at(j).m_IsThunder == true)
            {
                if (i == this->l_CityLen.lightLocation && j == this->l_CityWid.lightLocation)
                    cout << setw(2) << "+";
                else
